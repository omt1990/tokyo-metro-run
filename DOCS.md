...
## 7.5 ローカルテストプレイ（デモモード）

ゲーム作成→招待コード→チーム割り当ての手順を踏まずに、UIだけ1人で確認したい場合：

- 起動画面の左下「**テストプレイ**」ボタンを押す
- または URL に `?demo=1` を付けてアクセス（例：`https://tokyo-metro-run.web.app/?demo=1`）

挙動：

- ダブルビンゴモード・2チーム・自分はチームA所属・state=in_progress の状態で**ビンゴ画面に直接着地**
- 同意モーダルもスキップ（同意状態は `localStorage` に保存しない）
- **Firebase には一切書き込まない**（in-memory state 完結）。ページをリロードすると状態リセット
- ヘッダーに黄色の「DEMO」チップが出る
- 駅到着・ミッション記録・スコア計算・ビンゴ判定など全機能が動く

主な用途：

- UI 改修後の見た目チェック
- ミッションフローの確認（ステーション選択 → ミッション記録 → ビンゴ成立まで）
- スマホでのレイアウト確認

複数端末で同期テストしたい場合は通常通り招待コード経由で参加してください。
...
## 11. ルールの変遷（企画会議の記録・2024〜2025）

★ アプリになる前の「公式大会」で使われていたルールと、検討された追加要素の記録。
★★ 現在の実装（DOCS.md §1）との差分を見ると、**何を採り、何を捨てたか**が分かる。

### ★ 第1回 公式東京メトロラン（原型）

> 「あの東京メトロランがついに公式競技に！
> **1日乗車券**を用いて東京メトロを駆け巡りポイント争奪戦を行なって頂きます。
>
> 【基本ルール】
> - **2チーム**に分かれます
> - 各チームに **5×5マスに駅名が記載されているビンゴシート**を配布します
> - ★ **トランプで引いた数字の数だけ移動**ができます
> - ★ **上りに進むか下りに進むか選択可能**です
> - **乗り換え可能な駅に降りた場合、どの路線に（乗り換えるか選べる）**…」

★ **1日乗車券**が前提。トランプ、5×5ビンゴ、上り下りの選択——**現在の実装の骨格はここで完成している。**

### ★★ 第4回で検討された追加要素（★ ほとんど不採用）

> 「私が考えた要素です
> - ★ **最終的にはポイントで競う形式**
> - **ビンゴもポイントの加点要素の1つ**（ビンゴが成立したらプラス5ポイントとか）
> - ★★ **東京メトロの駅にはナンバリングがしてある。
>   例えば泊まった駅が奇数ならプラス、偶数ならマイナス**
> - ★★ **乗り換えを行うのには1ポイント使わないといけない**
>   （乗り換えとは、乗り換え可能な駅に停車し、今までいた路線から変えること）
> - ★ **東京を再発見するためのミッションがあり、それを停車駅でクリア**（する）」

### ★★★ 何が実装され、何が消えたか

| 検討された要素 | 現在の実装 |
|---|---|
| ★ ポイント制で競う | ★★ **採用**（駅ポイント × ミッションポイント） |
| ビンゴを加点要素の1つに | ★ **採用**（マス1pt＋ビンゴ1列3pt） |
| ★★ **駅ナンバリングの奇数/偶数で加減点** | ★★★ **不採用** |
| ★★ **乗り換えに1ポイント消費** | ★★★ **不採用** |
| ★ 東京を再発見するミッション | ★★ **採用**（ミッションビンゴ） |

★★★ **消えた2つは、どちらも「計算が増える」ルール。**
```
・奇数/偶数の加減点 → ★ 駅に着くたびに番号を確認して計算が要る
・乗り換えコスト     → ★ ポイントを支払う判断が毎回発生する
```

★★ これは `docs/product-mission.md` の判断基準そのもの:
> ★ **「説明しないと分からないルールになっていないか？」**

★★★ **面白そうな要素を思いついても、遊びの最中に計算が増えるものは落とす。**
実際に残ったのは「**駅ポイント × ミッションポイント**」という、
**最後に一度だけ掛け算するだけ**の形だった。

### ★ ミニゲーム案（すごろく版・第3回で検討）

停車駅で行う「進むための課題」の案:
```
ダーツでブルを当てる／ITO（ボードゲーム）を成功させる／
サイコロ3つでゾロ目／★ 東京メトロ駅前のジオゲッサー／
筋トレ◯◯回／ペットボトルフリップ／輪投げ／
★ 2010年以降の流行語クイズ
```

★★ **場所（駅前）に依存するもの（ジオゲッサー）と、
どこでもできるもの（筋トレ・フリップ）が混在している。**
★ 現在の「ミッション」の原型がこれ。

⚠️ **ジオゲッサー（Google Map の画像から駅を当てる）は、
外部画像の取得が必要でアプリに載せにくい。** 現在の実装に無いのはそのため（[推測]）。
